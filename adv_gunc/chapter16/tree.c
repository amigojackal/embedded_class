/* This demonstrates showing multi-dimensional data using a GtkTreeModel and
   GtkTreeView. */
...
int main (int argc, char *argv[])
{
  ...
  /* Create the tree store, passing in the number and type of each column*/                     
  store = gtk_tree_store_new (N_COLUMNS, G_TYPE_STRING,  G_TYPE_STRING, G_TYPE_STRING);

  /*  Create a new top-level row and acquire the row iterator */
  ...
  gtk_tree_store_set (store, &parent_iter, COLUMN_TITLE, "Dark Side of the Moon",
                                    ...
                                    -1);
  /* Next create a child row, acquiring the child row iterator */
